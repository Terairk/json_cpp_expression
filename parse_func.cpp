...
namespace json {
    ...
    while (index < tokens_size) {
      ...
      if (currentToken.type == JSONTokenType::Syntax) {
        ...
        // return an error
          ...
          return {{}, index, format_parse_error("Expected comma after element in array", currentToken)};
        }
      }
  ...
  // - Next token index to process
  // - Error message (empty string if successful)
  std::tuple<std::map<std::string, JSONValue>, int, std::string> parse_object(std::vector<JSONToken> tokens,
                                                                              int index) {
    ...
    while (index < tokens_size) {
      ...
      if (currentToken.type == JSONTokenType::Syntax) {
        ...
        // Invalid syntax at start of object
          ...
          return {{}, index, format_parse_error("Expected key-value pair or closing brace in object", currentToken)};
        }
      }
      ...
      if (!std::holds_alternative<std::string>(key.value)) {
        return {{}, index, format_parse_error("Expected string key in object", currentToken)};
      }
      index = new_index;
      currentToken = tokens[index];
      ...
      if (!(currentToken.type == JSONTokenType::Syntax && currentToken.value == ":")) {
        return {{}, index, format_parse_error("Expected colon after key in object", currentToken)};
      }
      index++;
